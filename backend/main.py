...
class Settings(BaseSettings):
    ...
    smtp_from_email: str  = ""
    frontend_url: str     = "https://tropi-care.vercel.app"
...
async def _fetch_clinic_elements(lat: float, lon: float) -> List[dict]:
    """
    Queries all upstream Overpass mirrors CONCURRENTLY rather than one at a
    time. This used to try each of the 3 CLINIC_DATA_SOURCES sequentially,
    each with its own CLINIC_SOURCE_TIMEOUT_S (12s) timeout — a worst case
    of ~36 seconds if the first two both failed or timed out. That is
    longer than the 30 second request timeout enforced by
    request_middleware below, so a slow/unreachable upstream mirror could
    cause this endpoint's own response to be replaced by that middleware's
    504 branch — a branch that (see the CORS section further down) never
    carries CORS headers, which is what surfaces in the browser simply as
    a blocked cross-origin request instead of a clear timeout.

    Racing all sources concurrently bounds the worst case to roughly one
    CLINIC_SOURCE_TIMEOUT_S window (~12s) instead of the sum of all of
    them, comfortably inside the outer 30 second budget. The first source
    to return a non-empty result wins and the rest are cancelled.
    """
    query = _build_clinic_query(lat, lon, HOSPITAL_SEARCH_RADIUS_M, LOCAL_SEARCH_RADIUS_M)
    async with aiohttp.ClientSession() as session:
        tasks = [
            asyncio.create_task(_query_clinic_source(session, source, query))
