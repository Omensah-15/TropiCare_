...
if _is_sqlite:
    engine = create_engine(
        ...
        pool_size=1,          # SQLite does not support concurrent writers
        max_overflow=0,
        pool_pre_ping=True,
    )
...
def load_ml_models() -> None:
    if not JOBLIB_AVAILABLE:
        logger.warning("joblib not installed — ML models unavailable; using built-in scoring engine.")
        return
    ...
    if not pkl_files:
        logger.warning("No .pkl files found in %s — using built-in scoring engine.", MODELS_DIR)
        return
    ...
    for fname in pkl_files:
        ...
        try:
            ...
            logger.info("ML model loaded: %s", fname)
        except Exception as exc:
            logger.error("Failed to load ML model %s: %s", fname, exc)

    # Validate that the ensemble trio is complete
    required = {"sctd_ensemble", "sctd_label_encoder", "sctd_feature_columns"}
    present = set(LOADED_MODELS.keys())
    missing = required - present
    if missing:
        logger.warning(
            "Incomplete ML model set — missing: %s. Scoring engine will be used.",
            missing,
        )
        # Remove partial models to avoid partial-use bugs
...
def get_next_question(answers: Dict[str, bool], asked: List[str]) -> Optional[Dict]:
    ranked = sorted(
...
# ─────────────────────────────────────────────────────────────
# ML INFERENCE — uses loaded models; scoring engine is fallback only
# ─────────────────────────────────────────────────────────────

def _scoring_engine_predict(answers: Dict[str, bool]) -> Dict[str, Any]:
    """
    Pure-Python scoring fallback used only when no ML models are available.
    """
...
def predict_with_ml(answers: Dict[str, bool]) -> Dict[str, Any]:
    """
    Run inference using the loaded scikit-learn ensemble when available.
    Falls back to the built-in scoring engine only when models are missing.
    """
    ...
    risk_override = LOADED_MODELS.get("sctd_risk_classification")
    ...
    if ensemble is not None and le is not None and cols is not None:
        try:
            ...
            disease = str(le.inverse_transform([top_idx])[0])
            confidence = float(proba[top_idx])

            effective_risk_map = risk_override if isinstance(risk_override, dict) else RISK_MAP
            all_probs = {
            ...
            logger.debug("ML prediction: %s (%.2f%%) via ensemble", disease, confidence * 100)
            return {
                "disease": disease,
                "confidence": confidence,
                "risk": effective_risk_map.get(disease, "Medium"),
                "all_scores": all_probs_sorted,
                "method": "ml",
            }

        except Exception as exc:
            logger.error("ML inference failed (%s) — falling back to scoring engine.", exc)

    return _scoring_engine_predict(answers)


# ─────────────────────────────────────────────────────────────
# HTTP CLIENT SINGLETON (shared across all requests)
# ─────────────────────────────────────────────────────────────
...
_REQUIRED_AI_FIELDS = ("explanation", "home_care", "test", "doctor", "safety")

_SYSTEM_PROMPT = (
    "You are a responsible clinical AI assistant for a tropical disease screening application "
    "serving patients in Ghana and West Africa. "
...
def _build_openrouter_prompt(
    ...
    urgency_map = {
        "High":   "URGENT — the patient must visit a hospital or clinic today",
        "Medium": "advise a clinic visit within one to two days if symptoms persist",
        "Low":    "advise rest at home and a clinic visit only if symptoms worsen",
    }
    urgency = urgency_map.get(risk, "advise a clinic visit")

    return f"""Patient reported symptoms: {sym_text}
Likely condition: {disease} (model confidence: {round(confidence * 100)}%)
Risk classification: {risk} — {urgency}
...
{{
  ...
  "home_care": "One to two specific things the patient can do at home right now.",
  "test": "One specific diagnostic test to confirm the condition, or 'No test required' if unnecessary.",
  "doctor": "One direct instruction — visit hospital, clinic, or pharmacy.",
  "safety": "One brief critical safety warning if risk is High; empty string otherwise."
}}

Rules:
- Use plain, simple language that a non-medical person can understand.
- Each field must be one sentence or a short phrase.
- For High risk: always instruct the patient to visit a hospital or clinic immediately.
- Do not invent drug names or dosages under any circumstances.
- Return only the JSON object — nothing before it, nothing after it."""


async def _single_openrouter_attempt(
    prompt: str,
    ...
    timeout = aiohttp.ClientTimeout(total=OPENROUTER_TIMEOUT, connect=8)
    async with session.post(OPENROUTER_URL, headers=headers, json=payload, timeout=timeout) as resp:
        if resp.status == 429:
            ...
            return None
        if resp.status == 402:
            logger.warning("OpenRouter credit limit reached (402).")
            return None
        if resp.status != 200:
            body = await resp.text()
            logger.warning("OpenRouter returned HTTP %d: %.300s", resp.status, body)
            return None
    ...
    if not raw:
        logger.warning("OpenRouter returned an empty content string.")
        return None
    ...
    # Locate JSON boundaries defensively
    brace_start = raw.find("{")
    brace_end = raw.rfind("}")
    if brace_start == -1 or brace_end == -1 or brace_end <= brace_start:
        logger.warning("Could not locate JSON object in OpenRouter response: %.200s", raw)
        return None
    ...
    try:
        ...
        logger.warning("JSON parse error in OpenRouter response (%s): %.200s", exc, raw)
        return None
    ...
    for field in _REQUIRED_AI_FIELDS:
        ...
        sanitised[field] = str(val).strip() if val else ""

    # Reject a response that is almost entirely empty
    populated = sum(1 for v in sanitised.values() if v)
    if populated < 3:
        logger.warning("OpenRouter response too sparse (%d/%d fields populated).", populated, len(_REQUIRED_AI_FIELDS))
        return None
...
async def call_openrouter(
    ...
    """
    Call OpenRouter with retry logic. Returns a sanitised dict or None.
    None means the caller should use the default recommendations.
    """
    if not OPENROUTER_API_KEY:
        logger.info("OPENROUTER_API_KEY is not configured — using default recommendations.")
        return None

    prompt = _build_openrouter_prompt(disease, risk, active_syms, confidence)
    session = _get_http_session()

    for attempt in range(1, OPENROUTER_MAX_RETRIES + 1):
        try:
            result = await _single_openrouter_attempt(prompt, session)
            if result is not None:
                logger.info(
                    "OpenRouter succeeded on attempt %d/%d for disease=%s risk=%s",
                    attempt, OPENROUTER_MAX_RETRIES, disease, risk,
                )
                return result
        except asyncio.TimeoutError:
            logger.warning(
                "OpenRouter attempt %d/%d timed out (%.1fs limit).",
                attempt, OPENROUTER_MAX_RETRIES, OPENROUTER_TIMEOUT,
            )
        except aiohttp.ClientError as exc:
            logger.warning(
                "OpenRouter attempt %d/%d network error: %s",
                attempt, OPENROUTER_MAX_RETRIES, exc,
            )
        except Exception as exc:
            logger.error(
                "OpenRouter attempt %d/%d unexpected error: %s",
                attempt, OPENROUTER_MAX_RETRIES, exc,
            )

        if attempt < OPENROUTER_MAX_RETRIES:
            await asyncio.sleep(1.0 * attempt)  # brief back-off between retries
...
def build_recommendation(
    ...
    if ai_result:
        # Merge AI result with defaults; AI content takes precedence when non-empty
        return {
...
async def lifespan(app: FastAPI):
    ...
    load_ml_models()
    # Warm the shared HTTP session
    _get_http_session()

    ml_status = (
        f"ensemble + label_encoder loaded ({len(LOADED_MODELS)} artefacts)"
        if "sctd_ensemble" in LOADED_MODELS
        else "scoring engine (no ML models)"
    )
    ai_status = (
        f"enabled — model={OPENROUTER_MODEL}"
        if OPENROUTER_API_KEY
        else "disabled (OPENROUTER_API_KEY not set)"
    )
    logger.info("TropiCare started  |  ML: %s  |  AI: %s", ml_status, ai_status)
    logger.info("Database: %s  |  Pool size: %s", DATABASE_URL.split("@")[-1], DB_POOL_SIZE)
    ...
    engine.dispose()
    logger.info("TropiCare shutdown complete.")
...
app = FastAPI(
    title="TropiCare API",
    version="2.0.0",
    description="AI-powered tropical disease symptom checker — KNUST Final Year Project",
    lifespan=lifespan,
    docs_url="/docs",
    redoc_url="/redoc",
)
...
async def health_check():
    return {
        ...
        "ml_inference": "ensemble" if "sctd_ensemble" in LOADED_MODELS else "scoring-engine",
...
async def next_question(
    ...
    if req.question_id not in Q_INDEX:
        raise HTTPException(status_code=400, detail=f"Unknown question_id: {req.question_id!r}")

    answers = _load_json(sess.answers, {})
    asked = _load_json(sess.asked_questions, [])

    answers[req.question_id] = req.answer
    if req.question_id not in asked:
        asked.append(req.question_id)

    sess.answers = _dump_json(answers)
    sess.asked_questions = _dump_json(asked)
    db.commit()
...
async def analyze(
    ...
    # ML / scoring inference (synchronous but fast; runs in main thread)
    pred = predict_with_ml(answers)

    # OpenRouter AI — await with hard timeout, graceful fallback
    ai_result: Optional[Dict[str, str]] = None
    if OPENROUTER_API_KEY:
        try:
            ai_result = await asyncio.wait_for(
                call_openrouter(pred["disease"], pred["risk"], active_syms, pred["confidence"]),
                timeout=OPENROUTER_TIMEOUT + 5,
            )
        except asyncio.TimeoutError:
            logger.warning("Global AI timeout exceeded for session=%s", session_id)
        except Exception as exc:
            logger.error("Unexpected error during AI call for session=%s: %s", session_id, exc)
    ...
    logger.info(
        "Diagnosis saved: id=%d user=%d disease=%s risk=%s confidence=%.2f method=%s ai_used=%s",
        diag.id, user_id, pred["disease"], pred["risk"],
        pred["confidence"], pred["method"], diag.ai_used,
    )
    ...
    return {
        ...
        "method": pred["method"],
        "ai_used": diag.ai_used,
    }


# ─────────────────────────────────────────────────────────────
# ROUTES — Patient History (per-user isolation enforced)
# ─────────────────────────────────────────────────────────────

@app.get("/api/v1/patient/history", tags=["Patient"])
...
@app.get("/api/v1/patient/history/{diag_id}", tags=["Patient"])
async def get_diagnosis(
    ...
    d = (
        db.query(DiagnosisModel)
        .filter(DiagnosisModel.id == diag_id, DiagnosisModel.user_id == user_id)
        .first()
    )
    if not d:
...
async def get_profile(
    ...
    high_risk = (
        db.query(DiagnosisModel)
        .filter(DiagnosisModel.user_id == user_id, DiagnosisModel.risk == "High")
        .count()
    )
...
@app.put("/api/v1/user/change-password", tags=["User"])
...
@app.delete("/api/v1/user/account", tags=["User"])
...
# ─────────────────────────────────────────────────────────────
# ROUTES — Admin (unauthenticated — restrict in production via reverse proxy)
# ─────────────────────────────────────────────────────────────
...
async def clear_database(db: Session = Depends(get_db)):
    ...
    db.commit()
    logger.warning("Admin cleared database: %d diagnosis records removed.", deleted)
    return {"message": f"Cleared {deleted} diagnosis record(s)."}
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        ...
        loop="uvloop" if os.getenv("USE_UVLOOP", "0") == "1" else "auto",
        reload=os.getenv("RELOAD", "0") == "1",
        log_level="info",
        access_log=True,
        limit_concurrency=int(os.getenv("LIMIT_CONCURRENCY", "500")),
