...
def _who_don_http_session() -> aiohttp.ClientSession:
    """
    Builds the aiohttp session used to call WHO, raising the
    max_line_size/max_field_size header limits (aiohttp defaults to 8190
    bytes each) so who.int's oversized Content-Security-Policy response
    header doesn't abort the request before the body is ever read. Falls
    back to aiohttp's defaults on older aiohttp versions that don't
    accept these constructor kwargs -- the caller's own timeout/stale-
    cache handling already covers a fetch failing either way.
    """
    try:
        return aiohttp.ClientSession(max_line_size=64 * 1024, max_field_size=64 * 1024)
    except TypeError:
...
async def _fetch_who_don_raw() -> List[dict]:
    ...
    who.int fronts this endpoint with a very large Content-Security-Policy
    response header (well over aiohttp's default 8190-byte line/field
    limit), which made every request fail with "Got more than 8190 bytes
    when reading" before the response body was ever read -- the feed
    never populated even though WHO itself was reachable and healthy.
    _who_don_http_session() raises that limit so the request goes through.
    """
    params = {
        "sf_culture": "en",
